...
val newTe = knownAssembler.transform(knownTest2).select("features", "returnquantity")
...
val model = NaiveBayes.train(labeledKnownTr, lambda = 1.0, modelType = "multinomial")
